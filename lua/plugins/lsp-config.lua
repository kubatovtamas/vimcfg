...
return {
	...
	},
	{
		...
		config = function()
			...
			local on_attach_base = function(client, bufnr, enable_formatting)
				...
				vim.keymap.set(
					...
					merge_tables(bufopts, { desc = "Code [A]ction" })
				)
				...
				vim.keymap.set("n", "<leader>K", vim.lsp.buf.signature_help, merge_tables(bufopts, {desc = "Signature Help"}))
			...
			lspconfig.pyright.setup({
				...
				on_attach = function(client, bufnr)
					-- Explicitly pass false to disable formatting
					on_attach_base(client, bufnr, false)
				end,
			})
